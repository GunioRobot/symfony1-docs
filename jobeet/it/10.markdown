...
>**TIP**
>Si può fondere un form in un altro usando il metodo `mergeForm()` o incorporarne
...
è stato lanciato il task `propel:build --all` durante il giorno 3, symfony ha chiamato
...
ha `JobeetJobForm`) che inizialmente è vuota, visto che eredita la struttura dalla
...
>(`full-time` per esempio), si può cambiare il valore predefinito nello schema
>del database.

Anche se si ritiene che nessuno possa inviare un valore non valido, un hacker potrebbe
bypassare facilmente le opzioni del widget, usando strumenti come [curl](http://curl.haxx.se/)
...
>**NOTE**
>Occorre creare la cartella dei loghi (`web/uploads/jobs/`) e verificare
...
>**NOTE**
>Se non si è ancora aggiunto il foglio di stile `job`, è ora di farlo
>in entrambi i template (`<?php use_stylesheet('job.css') ?>`).

Il form stesso è visualizzato nel ~partial|Partial~ `_form`. Sostituire
...
>d'aiuto se in seguito si decide di cambiare un widget con uno che necessita di
